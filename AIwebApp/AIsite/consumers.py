    ...
    async def chat_message(self, event):
        if await cur_balance(self.scope['session']['user_pk']):
            ...
            if chat_slug != 'not_defined':
                await create_msg(chat_slug, 'user', event['event_data']['message'])  # saves new msg to db
                await self.send(text_data=json.dumps(                             # sends your message back to frontend to add in DOM
                    {'type': 'chat.message',
                     'event_data': data,
                     }
                ))
